...
| P2 | 59 | Deuda / limpieza |
| INFO | 24 | Verificado, aceptado o límite de cobertura |

## Anexos

| Anexo | P0 | P1 | P2 | INFO |
|---|---|---|---|---|
| [Anexo A · Rutas montadas vs. ROUTE_PERMISSIONS](anexos/01-routes-vs-permissions.md) | 0 | 0 | 16 | 17 |
| [Anexo B · Endpoints consumidos por la SPA vs. rutas montadas](anexos/02-frontend-vs-backend.md) | 0 | 0 | 18 | 2 |
| [Anexo C · Consistencia de vistas (registro, navegación, permisos, build)](anexos/03-views-consistency.md) | 0 | 0 | 0 | 1 |
| [Anexo D · Consultas del backend vs. esquema real de la base de datos](anexos/04-db-vs-code.md) | 0 | 0 | 8 | 0 |
| [Anexo E · Modelo de autorización: código vs. base de datos](anexos/05-permissions-db-drift.md) | 0 | 0 | 8 | 1 |
| [Anexo G · Código, archivos y dependencias sin uso](anexos/06-dead-code.md) | 0 | 0 | 1 | 3 |
...
| AUD-RP020 | config-muerta | Entrada de ROUTE_PERMISSIONS sin ruta viva: PATCH /api/v1/usuarios | src/config/permissions.js:9 | Eliminar la entrada o corregir la clave si la ruta cambió de forma. |
| AUD-RP021 | config-muerta | Entrada de ROUTE_PERMISSIONS sin ruta viva: PUT /api/v1/ventas | src/config/permissions.js:27 | Eliminar la entrada o corregir la clave si la ruta cambió de forma. |
| AUD-RP022 | config-muerta | Entrada de ROUTE_PERMISSIONS sin ruta viva: GET /api/v1/cuotas | src/config/permissions.js:31 | Eliminar la entrada o corregir la clave si la ruta cambió de forma. |
| AUD-RP023 | config-muerta | Entrada de ROUTE_PERMISSIONS sin ruta viva: PUT /api/v1/cuotas | src/config/permissions.js:36 | Eliminar la entrada o corregir la clave si la ruta cambió de forma. |
| AUD-RP024 | config-muerta | Entrada de ROUTE_PERMISSIONS sin ruta viva: PUT /api/v1/facturas | src/config/permissions.js:50 | Eliminar la entrada o corregir la clave si la ruta cambió de forma. |
| AUD-RP025 | config-muerta | Entrada de ROUTE_PERMISSIONS sin ruta viva: GET /api/v1/reportes | src/config/permissions.js:70 | Eliminar la entrada o corregir la clave si la ruta cambió de forma. |
| AUD-RP026 | config-muerta | Entrada de ROUTE_PERMISSIONS sin ruta viva: GET /api/v1/reportes/dir | src/config/permissions.js:71 | Eliminar la entrada o corregir la clave si la ruta cambió de forma. |
| AUD-RP027 | config-muerta | Entrada de ROUTE_PERMISSIONS sin ruta viva: GET /api/v1/reportes/jur | src/config/permissions.js:72 | Eliminar la entrada o corregir la clave si la ruta cambió de forma. |
| AUD-RP028 | config-muerta | Entrada de ROUTE_PERMISSIONS sin ruta viva: GET /api/v1/mi-cuenta | src/config/permissions.js:86 | Eliminar la entrada o corregir la clave si la ruta cambió de forma. |
| AUD-RP029 | config-muerta | Entrada de ROUTE_PERMISSIONS sin ruta viva: PATCH /api/v1/config | src/config/permissions.js:143 | Eliminar la entrada o corregir la clave si la ruta cambió de forma. |
...
| AUD-DB001 | bd-esquema-inconsistente | RPC que vive en public y no en condor: next_consecutivo_condor() | src/controllers/facturas.controller.js:14 | Documentar la excepción, o mover la función a `condor` y añadir `.schema('condor')` en los 5 puntos de llamada. |
| AUD-DB002 | bd-sin-uso | Objeto de BD que el backend nunca consulta: consecutivos | condor.consecutivos | Confirmar si sigue siendo necesario; documentarlo o retirarlo. |
| AUD-DB003 | bd-sin-uso | Objeto de BD que el backend nunca consulta: vw_auditoria_juridica | condor.vw_auditoria_juridica | Confirmar si sigue siendo necesario; documentarlo o retirarlo. |
| AUD-DB004 | bd-sin-uso | Objeto de BD que el backend nunca consulta: vw_dir_auditoria | condor.vw_dir_auditoria | Confirmar si sigue siendo necesario; documentarlo o retirarlo. |
| AUD-DB005 | bd-sin-uso | Objeto de BD que el backend nunca consulta: vw_dir_recaudo_facturacion_hoy | condor.vw_dir_recaudo_facturacion_hoy | Confirmar si sigue siendo necesario; documentarlo o retirarlo. |
| AUD-DB006 | bd-sin-uso | Objeto de BD que el backend nunca consulta: vw_disponibilidad_comercial | condor.vw_disponibilidad_comercial | Confirmar si sigue siendo necesario; documentarlo o retirarlo. |
| AUD-DB007 | bd-sin-uso | Objeto de BD que el backend nunca consulta: whatsapp_conversacion | condor.whatsapp_conversacion | Confirmar si sigue siendo necesario; documentarlo o retirarlo. |
| AUD-DB008 | bd-sin-uso | Objeto de BD que el backend nunca consulta: whatsapp_mensaje | condor.whatsapp_mensaje | Confirmar si sigue siendo necesario; documentarlo o retirarlo. |
...
| AUD-PD003 | permiso-muerto | Permiso que ninguna ruta exige y ninguna vista otorga: notificaciones_jur:generar | condor.permisos | Confirmar si se valida dentro de algún controller; si no, eliminar el permiso y sus concesiones. |
| AUD-PD004 | permiso-muerto | Permiso que ninguna ruta exige y ninguna vista otorga: notificaciones_jur:leer | condor.permisos | Confirmar si se valida dentro de algún controller; si no, eliminar el permiso y sus concesiones. |
| AUD-PD005 | permiso-muerto | Permiso que ninguna ruta exige y ninguna vista otorga: notificaciones_jur:reenviar | condor.permisos | Confirmar si se valida dentro de algún controller; si no, eliminar el permiso y sus concesiones. |
| AUD-PD006 | permiso-muerto | Permiso de visibilidad de una vista que ya no existe: vista:inventario | condor.permisos | Eliminar el permiso y sus concesiones, o registrar la vista si debía existir. |
| AUD-PD007 | permiso-muerto | Permiso que ninguna ruta exige y ninguna vista otorga: inventario:leer | condor.permisos | Confirmar si se valida dentro de algún controller; si no, eliminar el permiso y sus concesiones. |
| AUD-PD008 | permiso-muerto | Permiso que ninguna ruta exige y ninguna vista otorga: requerimientos:entregar | condor.permisos | Confirmar si se valida dentro de algún controller; si no, eliminar el permiso y sus concesiones. |
| AUD-DC004 | script-ad-hoc | 5 scripts de mantenimiento puntual en seeds/ | seeds/backfill_codigo_venta.js<br>seeds/diag-cuotas-comprador.js<br>seeds/gen-bank-tx-comprador.js<br>seeds/reset-comprador.js<br>seeds/reset-pagos-comprador.js | Mover a `tools/dev/` (fuera del runner de seeds) o eliminar antes de la entrega. |
