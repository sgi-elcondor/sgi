...
| `gasto`                | Gastos operativos por proyecto y categoría. `id_empresa` opcional vincula a una empresa aliada (ALI-02). |
| `empresa_aliada`       | Proveedores/socios/contratistas (ALI-01): `razon_social`, `nit` (único, DV DIAN validado en backend), `rup`, `codigos_actividad text[]` (UNSPSC/CIIU), `tipo`, datos de contacto, `activo`. `gasto.id_empresa` y `requerimiento.id_empresa` (seteado en el desembolso) centralizan el historial comercial (ALI-02). |
...
### Empresas aliadas (ALI-01 / ALI-02)
- `GET /empresas-aliadas?q=&tipo=&activas=1` — catálogo (búsqueda por razón social/NIT/RUP; usado por los autocompletados).
- `GET /empresas-aliadas/:id` — detalle + **historial comercial**: gastos y requerimientos desembolsados vinculados, con totales.
- `POST /` y `PUT /:id` — registro/edición con validaciones de `empresas.service`: NIT con **dígito de verificación DIAN**, RUP (número de inscripción, solo dígitos), códigos de actividad (2-8 dígitos c/u). NIT único (409 en duplicado). Auditado.
- Vínculo ALI-02: `POST/PUT /gastos` y `PATCH /requerimientos/:id/desembolsar` aceptan `id_empresa` opcional (validado contra empresa activa); el gasto automático del desembolso hereda el vínculo y la trazabilidad muestra el proveedor. Frontend: autocompletado (`SGIEmpresas.wireEmpresaAutocomplete`) en los formularios de gasto y desembolso.
- Permisos: `empresas_aliadas:leer/crear/actualizar`; vista `empresas-aliadas` (grupo Operación). Los roles con `gastos:crear` o `requerimientos:desembolsar` reciben `leer` vía migración.
...
- `PATCH /config/:clave` — Body: `{ valor }`. Sólo claves whitelisted en `config.controller.EDITABLE_KEYS` (hoy `umbral_compra_grande` y `umbral_caja_menor`). Auditado. **POL-04**: validación cruzada — `umbral_caja_menor` debe ser estrictamente menor que `umbral_compra_grande` (422 si se cruzan), así las tres categorías (caja menor / estándar / grande) nunca se solapan. El admin edita ambos topes desde la card **"Políticas de compras"** de la vista Permisos, que muestra los rangos derivados de cada categoría.
...
- **Toda nueva ruta** se registra en `src/config/permissions.js` con el prefijo `/api/v1/` y la forma `'MÉTODO /api/v1/ruta': { recurso, accion }`. Si la ruta no aparece, basta con estar autenticado. **Trampa (SEG-09)**: el middleware solo elimina segmentos **numéricos** (`/\d+`) al construir la clave; una ruta con parámetro no numérico (ej. `/config/:clave`) jamás coincide con el mapa y queda abierta a cualquier autenticado — en esos casos el permiso se valida **dentro del controller** (ver `_puedeConfig` en `config.controller.js`).
