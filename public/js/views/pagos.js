...
function _facturasListaHTML(facturas, filtro) {
  const q = _pagosNorm(filtro);
  const visibles = q
    ? facturas.filter(f => _pagosNorm(f.comprador).includes(q) || _pagosNorm(f.codigo_lote).includes(q))
    : facturas;
        ...
        ${g.facturas.map(f => `
          <label style="display:flex;align-items:center;gap:10px;padding:6px 8px;border-top:1px solid var(--border);cursor:pointer">
            ...
            <span style="flex:1;font-size:.83rem;display:flex;justify-content:space-between;align-items:center;gap:8px;flex-wrap:wrap">
              <span>
                Factura <strong>${_fmtFactNum(f.numero_factura)}</strong>
...
window._filtrarFacturasPago = function() {
  ...
  if (!lista) return;
  lista.innerHTML = _facturasListaHTML(
...
async function _loadPagosView(puedeEscribir) {
  ...
  vc.innerHTML = `
    <div class="table-wrap">
      <div class="table-header">
        ...
        ${puedeEscribir ? `<button class="btn btn-primary btn-sm" onclick="pagoForm()">+ Registrar Pago</button>` : ""}
      </div>
  ...
  function aplicarFiltros() {
    ...
    const fComp     = _pagosNorm(document.getElementById("pv-comprador").value);
    const visibles  = grupos.filter(g => {
      if (fProyecto && g.proyecto !== fProyecto)               return false;
      if (fComp     && !_pagosNorm(g.comprador).includes(fComp)) return false;
      return true;
    });
  ...
  tbody.addEventListener("click", e => {
    ...
    if (g) pagosDeVentaView(g, puedeEscribir);
  });
}

window.pagosReadView = async function() { await _loadPagosView(false); };
window.pagosEditView = async function() { await _loadPagosView(true); };

// ── Vista detalle: pagos de una venta ─────────────────────────────────────────
window.pagosDeVentaView = function(grupo, puedeEscribir) {
  const vc = document.getElementById("viewContainer");
  window._pagosDetalleGrupo = grupo;

  if (puedeEscribir === undefined) {
    const rol = window.currentUser?.rol;
    puedeEscribir = rol === "auxiliar_contable" || rol === "admin";
  }
  ...
  vc.innerHTML = `
    <div class="table-wrap">
      <div class="table-header">
        ...
        ${puedeEscribir ? `<button class="btn btn-primary btn-sm" onclick="pagoForm(${grupo.id_venta ?? "null"})">+ Registrar Pago</button>` : ""}
      </div>
...
window._volverPagosView = function() {
  const rol = window.currentUser?.rol;
  (rol === "auxiliar_contable" || rol === "admin") ? pagosEditView() : pagosReadView();
};
