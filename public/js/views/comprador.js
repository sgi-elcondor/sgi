  ...
  window.compradorRecibosView = async function (container) {
    ...
    const recibosHtml = (misRecibos || []).length
      ...
      : `<div style="padding:48px 24px;text-align:center;color:var(--text-muted)">
          <div style="margin-bottom:12px;opacity:.4">${window.SGIUI?.icon("receipt") ?? ""}</div>
          <p style="font-size:.95rem;font-weight:600;color:var(--text);margin-bottom:4px">Sin recibos aun</p>
    ...
    vc.innerHTML = `
      <section class="page-shell">
        ${window.SGIUI?.pageHeader({
          kicker: "Mi cuenta",
          title: "Mis Recibos",
          subtitle: "Recibos de pago emitidos a tu nombre.",
